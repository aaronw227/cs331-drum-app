# metronome.py
# This is a simple metronome with a GUI using tkinter and using pygame for sound playback.
...
def start_metronome():
    # worker is a global variable to keep track of the metronome thread
    global worker
    ...
    try:
        ...
        status_label.config(text="Enter a valid BPM (e.g., 120)")
        return

    # worker is already running, do nothing
    if worker and worker.is_alive():
        return

    # Clear the stop event and start the metronome thread
    stop_event.clear()
    status_label.config(text=f"Running at {bpm} BPM")
...
status_label = tk.Label(root, text="Stopped"); status_label.pack(pady=5)
